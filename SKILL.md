...
2. 禁止擅自决定位置与暴露系统内部路径 — 用户已指定存放目录则直接使用；未指定时必须仅使用一句话自然询问用户：“请问测试工程保存到哪个目录？（例如：api-test-suite）”，严禁擅自推断或补充任何系统默认路径，严禁暴露系统底层内部路径（如包含 /workspaces/、/tmp/ 或环境 ID 的系统路径），获取明确路径后方可创建目录；测试范围与认证方式经确认后方可写代码
