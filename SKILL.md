...
请确认：
1. 走哪条路径（快速流程 / 完整流程）？
2. 测试工程保存到哪个目录？（例如：~/Desktop/my-api-tests）
```
...
⚠️ **前置绝对铁门禁（严禁跳过）**：
- **检查是否已有保存目录**：
  - 用户已提供目录 → 确认为 `<PROJECT_DIR>`，继续执行。
  - **用户未提供保存目录（例如用户仅回复了“走快速路径”但未说明存哪）**：
    → **⛔ 必须停下来单问一句**：“请问测试工程保存到哪个目录？（例如：~/Desktop/my-api-tests）”
    → **在用户明确回复路径前：严禁读取任何实现模板！严禁自作主张声称“我先在工作区搭建测试框架”！严禁创建任何目录与文件！必须停下来等待用户回复！**
- 目录确认后：
  - 快速路径 → 读取 `reference/stages/stage-quick-setup.md`（⚠️ 严禁此时提前读取 implementation.md）
...
2. 禁止擅自决定位置与跳步执行 — 用户已指定存放目录则直接使用；未指定时必须停下来单问一句：“请问测试工程保存到哪个目录？（例如：~/Desktop/my-api-tests）”，收到用户回复前严禁执行任何写操作。严禁在用户未提供路径时自作主张声称“我先在工作区搭建测试框架”，严禁暴露系统底层内部路径（如包含 /workspaces/、/tmp/ 或环境 ID 的路径），严禁在未确认目录前生成任何测试代码；测试范围与认证方式经确认后方可写代码
